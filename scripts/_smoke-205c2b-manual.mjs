#!/usr/bin/env node
/**
 * Sprint 20.5C.2B — Manual smoke: Bulk Role Change.
 */
import { join, dirname } from "node:path";
import { fileURLToPath } from "node:url";
import dotenv from "dotenv";
import { chromium } from "playwright";

const root = join(dirname(fileURLToPath(import.meta.url)), "..");
dotenv.config({ path: join(root, ".env.local") });

const BASE_URL = process.env.SMOKE_BASE_URL ?? "http://localhost:3000";
const EMAIL = process.env.SMOKE_OWNER_EMAIL ?? "wlasciciel@piorun.test";
const PASS = process.env.SETUP_TEST_PASSWORD ?? "Piorun2026!";

const COACH_LABEL = "Trener";
const results = [];
const bulkSamples = [];
let initialSnapshot = [];

function record(id, verdict, detail) {
  results.push({ id, verdict, detail });
  console.log(`[${verdict}] ${id} — ${detail}`);
}

async function login(page) {
  await page.goto(`${BASE_URL}/login`, { waitUntil: "domcontentloaded", timeout: 60000 });
  await page.fill('input[name="email"], input[type="email"]', EMAIL);
  await page.fill('input[name="password"], input[type="password"]', PASS);
  await page.click('button[type="submit"]');
  await page.waitForURL((url) => !url.pathname.includes("/login"), { timeout: 30000 });
}

async function gotoMembers(page) {
  await page.goto(`${BASE_URL}/members`, { waitUntil: "networkidle", timeout: 60000 });
}

function membersTable(page) {
  return page
    .locator("table")
    .filter({ has: page.getByRole("columnheader", { name: "Imię i nazwisko" }) })
    .first();
}

async function scrapeMembers(page) {
  const rows = membersTable(page).locator("tbody tr");
  const count = await rows.count();
  const out = [];
  for (let index = 0; index < count; index++) {
    const row = rows.nth(index);
    const cells = row.locator("td");
    out.push({
      index,
      name: (await cells.nth(1).textContent())?.trim() ?? "",
      role: (await cells.nth(3).textContent())?.trim() ?? "",
      status: (await cells.nth(4).textContent())?.trim() ?? "",
    });
  }
  return out;
}

async function clearSelection(page) {
  const header = membersTable(page).locator('thead input[type="checkbox"]');
  if (await header.isChecked()) {
    await header.click();
    await page.waitForTimeout(150);
    await header.click();
  }
}

async function selectRowsByIndex(page, indices) {
  const checks = membersTable(page).locator('tbody input[type="checkbox"]');
  for (const i of indices) {
    const box = checks.nth(i);
    if (!(await box.isChecked())) await box.click();
  }
}

async function dismissBulkResultPanel(page) {
  const panel = page
    .getByRole("status")
    .filter({ hasText: /Zmieniono rolę|Zawieszono|Przywrócono/ });
  if ((await panel.count()) === 0) return;
  const closeBtn = panel.first().getByRole("button", { name: "Zamknij" });
  if ((await closeBtn.count()) > 0) {
    await closeBtn.click();
    await panel.first().waitFor({ state: "hidden", timeout: 5000 }).catch(() => {});
  }
}

function isOwnerMember(member) {
  return member.role === "Właściciel";
}

function parseRoleBulkSummary(text) {
  const noneChanged = /Zmieniono rolę 0 z (\d+)/.exec(text);
  if (noneChanged) {
    const total = +noneChanged[1];
    const skippedMatch = text.match(/(\d+) pominięto/);
    const failedMatch = text.match(/(\d+) nie udało się/);
    return {
      succeeded: 0,
      total,
      skipped: skippedMatch ? +skippedMatch[1] : total,
      failed: failedMatch ? +failedMatch[1] : 0,
      raw: text,
    };
  }
  const fullPlural = text.match(/Zmieniono rolę (\d+) członków/);
  if (fullPlural) {
    return { succeeded: +fullPlural[1], total: +fullPlural[1], skipped: 0, failed: 0, raw: text };
  }
  const fullSingular = text.match(/Zmieniono rolę (\d+) członka/);
  if (fullSingular) {
    return { succeeded: +fullSingular[1], total: +fullSingular[1], skipped: 0, failed: 0, raw: text };
  }
  const partial = text.match(/Zmieniono rolę (\d+) z (\d+)/);
  if (partial) {
    const succeeded = +partial[1];
    const total = +partial[2];
    const skippedMatch = text.match(/(\d+) pominięto/);
    const failedMatch = text.match(/(\d+) nie udało się/);
    return {
      succeeded,
      total,
      skipped: skippedMatch ? +skippedMatch[1] : 0,
      failed: failedMatch ? +failedMatch[1] : 0,
      raw: text,
    };
  }
  return { succeeded: -1, total: -1, skipped: -1, failed: -1, raw: text };
}

async function runBulkRoleChange(page, targetRoleLabel) {
  await page.locator("div.bg-muted\\/20").getByRole("button", { name: /^Zmień rolę/ }).first().click();
  const dialog = page.getByRole("dialog");
  await dialog.waitFor({ state: "visible", timeout: 5000 });
  if (targetRoleLabel) {
    await dialog.locator('select[name="role"]').selectOption({ label: targetRoleLabel });
  }
  const postPromise = page.waitForResponse(
    (resp) => resp.url().includes("/members") && resp.request().method() === "POST",
    { timeout: 20000 },
  );
  await dialog.locator('button[type="submit"]').click();
  await postPromise;
  await dialog.waitFor({ state: "hidden", timeout: 10000 }).catch(() => {});
  const statusPanel = page.getByRole("status").filter({ hasText: "Zmieniono rolę" });
  await statusPanel.waitFor({ state: "visible", timeout: 15000 });
  const summary = (await statusPanel.textContent())?.trim() ?? "";
  const parsed = parseRoleBulkSummary(summary);
  bulkSamples.push({ targetRoleLabel, ...parsed, summary });
  return parsed;
}

async function readDownload(download) {
  const stream = await download.createReadStream();
  const chunks = [];
  for await (const chunk of stream) chunks.push(chunk);
  return Buffer.concat(chunks).toString("utf8");
}

async function main() {
  const browser = await chromium.launch({ headless: true });
  const context = await browser.newContext({ acceptDownloads: true });
  const page = await context.newPage();

  const consoleErrors = [];
  const hydrationErrors = [];
  page.on("console", (msg) => {
    if (msg.type() === "error") consoleErrors.push(msg.text());
  });
  page.on("pageerror", (err) => hydrationErrors.push(err.message));

  try {
    await login(page);
    await gotoMembers(page);
    let members = await scrapeMembers(page);
    initialSnapshot = members.map((m) => ({ name: m.name, role: m.role }));

    // TEST A — bulk role change → coach (Trener)
    const changeable = members
      .filter((m) => !isOwnerMember(m) && m.status === "Aktywny" && m.role !== COACH_LABEL)
      .slice(0, 2);
    if (changeable.length < 2) {
      const fallback = members
        .filter((m) => !isOwnerMember(m) && m.status === "Aktywny")
        .slice(0, 2);
      changeable.push(...fallback.slice(changeable.length));
    }
    if (changeable.length >= 2) {
      await clearSelection(page);
      await selectRowsByIndex(page, changeable.map((m) => m.index));
      const beforeRoles = changeable.map((m) => m.role);
      const parsed = await runBulkRoleChange(page, COACH_LABEL);
      const ok = parsed.succeeded === 2 && parsed.skipped === 0 && parsed.failed === 0;
      record(
        "TEST-A",
        ok ? "PASS" : "FAIL",
        `coach bulk result=${JSON.stringify(parsed)} members=${changeable.map((m) => m.name).join(", ")}`,
      );
      await page.waitForTimeout(1500);
      await gotoMembers(page);
      members = await scrapeMembers(page);
      const afterRoles = changeable.map((m) => members[m.index]?.role);
      const tableOk = afterRoles.every((r) => r === COACH_LABEL);
      record(
        "TEST-A-table",
        tableOk ? "PASS" : "FAIL",
        `before=[${beforeRoles.join(",")}] after=[${afterRoles.join(",")}]`,
      );
      await dismissBulkResultPanel(page);
    } else {
      record("TEST-A", "FAIL", `tylko ${changeable.length} active non-owner — wymagane 2`);
      record("TEST-A-table", "SKIP", "");
    }

    // TEST B — owner exclusion + member changed
    await gotoMembers(page);
    members = await scrapeMembers(page);
    const ownerRow = members.find((m) => isOwnerMember(m));
    const nonOwner = members.find(
      (m) => !isOwnerMember(m) && m.status === "Aktywny" && m.role !== "Zawodnik",
    );
    if (ownerRow && nonOwner) {
      await clearSelection(page);
      await selectRowsByIndex(page, [ownerRow.index, nonOwner.index]);
      const toolbarLabel = await page
        .locator("div.bg-muted\\/20")
        .getByRole("button", { name: /^Zmień rolę/ })
        .first()
        .textContent();
      const hint = (await page.getByText("Właściciel wykluczony z operacji zbiorczych").count()) > 0;
      const ownerRoleBefore = ownerRow.role;
      const memberRoleBefore = nonOwner.role;
      const parsed = await runBulkRoleChange(page, "Zawodnik");
      await page.waitForTimeout(1500);
      await gotoMembers(page);
      members = await scrapeMembers(page);
      const ownerAfter = members[ownerRow.index]?.role;
      const memberAfter = members[nonOwner.index]?.role;
      const ok =
        toolbarLabel?.includes("Zmień rolę (1)") &&
        hint &&
        parsed.succeeded === 1 &&
        parsed.total === 1 &&
        parsed.skipped === 0 &&
        ownerAfter === ownerRoleBefore &&
        memberAfter === "Zawodnik";
      record(
        "TEST-B",
        ok ? "PASS" : "FAIL",
        `toolbar=${toolbarLabel} hint=${hint} result=${JSON.stringify(parsed)} owner=${ownerAfter} member=${memberRoleBefore}->${memberAfter}`,
      );
      await dismissBulkResultPanel(page);
      await clearSelection(page);
    } else {
      record("TEST-B", "SKIP", "brak owner lub eligible non-owner");
    }

    // TEST C — no-op: 2× coach → coach
    await gotoMembers(page);
    members = await scrapeMembers(page);
    const coachRows = members
      .filter((m) => !isOwnerMember(m) && m.status === "Aktywny" && m.role === COACH_LABEL)
      .slice(0, 2);
    if (coachRows.length >= 2) {
      await clearSelection(page);
      await selectRowsByIndex(page, coachRows.map((m) => m.index));
      const parsed = await runBulkRoleChange(page, COACH_LABEL);
      const detailsBtn = page.getByRole("button", { name: "Pokaż szczegóły" });
      const hasDetails = (await detailsBtn.count()) > 0;
      let detailText = "";
      if (hasDetails) {
        await detailsBtn.click();
        detailText = (await page.getByRole("status").textContent()) ?? "";
      }
      const ok =
        parsed.succeeded === 0 &&
        parsed.skipped === 2 &&
        parsed.total === 2 &&
        detailText.includes("Rola bez zmian");
      record(
        "TEST-C",
        ok ? "PASS" : "FAIL",
        `result=${JSON.stringify(parsed)} details=${detailText.includes("Rola bez zmian")}`,
      );
      await dismissBulkResultPanel(page);
      await clearSelection(page);
    } else {
      record("TEST-C", "FAIL", `tylko ${coachRows.length} coach — wymagane 2`);
    }

    // TEST D — single-row role change
    await gotoMembers(page);
    members = await scrapeMembers(page);
    const single = members.find(
      (m) => !isOwnerMember(m) && m.status === "Aktywny" && m.role !== "Skarbnik",
    );
    if (single) {
      const row = membersTable(page).locator("tbody tr").nth(single.index);
      const currentRole = single.role;
      const newRoleLabel = "Skarbnik";
      await row.locator('[aria-label="Akcje członka"]').click();
      await page.getByRole("menuitem", { name: "Zmień rolę" }).click();
      const dialog = page.getByRole("dialog");
      await dialog.waitFor({ state: "visible" });
      await dialog.locator('select[name="role"]').selectOption({ label: newRoleLabel });
      await dialog.getByRole("button", { name: "Zapisz rolę" }).click();
      await page.waitForTimeout(2000);
      await gotoMembers(page);
      members = await scrapeMembers(page);
      const updated = members[single.index]?.role === newRoleLabel;
      record(
        "TEST-D",
        updated ? "PASS" : "FAIL",
        `${single.name}: ${currentRole} -> ${members[single.index]?.role} expected=${newRoleLabel}`,
      );
    } else {
      record("TEST-D", "SKIP", "brak active non-owner");
    }

    // TEST E — CSV export
    await gotoMembers(page);
    await clearSelection(page);
    const [dlAll] = await Promise.all([
      page.waitForEvent("download", { timeout: 10000 }),
      page.getByRole("button", { name: /Eksportuj wszystkich/ }).click(),
    ]);
    const csvAll = await readDownload(dlAll);
    const allRows = csvAll.replace(/^\uFEFF/, "").split(/\r?\n/).length - 1;
    const memberCount = (await scrapeMembers(page)).length;
    await selectRowsByIndex(page, [0, 1].filter((i) => i < memberCount));
    const [dlSel] = await Promise.all([
      page.waitForEvent("download", { timeout: 10000 }),
      page.getByRole("button", { name: /Eksportuj zaznaczone/ }).click(),
    ]);
    const csvSel = await readDownload(dlSel);
    const selRows = csvSel.replace(/^\uFEFF/, "").split(/\r?\n/).length - 1;
    record(
      "TEST-E",
      allRows >= 1 && selRows === 2 && csvAll.includes("Imię i nazwisko") ? "PASS" : "FAIL",
      `allRows=${allRows} selRows=${selRows}`,
    );

    // TEST F — invitations
    await page.getByRole("button", { name: /Zaproszenia/ }).click();
    const hasFilters = (await page.getByText("Wymaga działania").count()) > 0;
    const hasTable = (await page.locator("table").count()) >= 1;
    const hasActions =
      (await page.getByRole("button", { name: /Ponów|Anuluj|Zaproś/ }).count()) >= 0;
    record(
      "TEST-F",
      hasFilters && hasTable ? "PASS" : "FAIL",
      `filters=${hasFilters} table=${hasTable} actionsSurface=${hasActions}`,
    );

    // TEST G — runtime
    await page.goto(`${BASE_URL}/members`, { waitUntil: "networkidle" });
    await page.reload({ waitUntil: "networkidle" });
    const hydra = hydrationErrors.filter((e) => /hydration|did not match/i.test(e));
    const noisyConsole = consoleErrors.filter(
      (e) => !/favicon|404|Failed to load resource/i.test(e),
    );
    record(
      "TEST-G",
      hydra.length === 0 && noisyConsole.length === 0 ? "PASS" : "FAIL",
      `hydration=${hydra.join("|") || "none"} console=${noisyConsole.slice(0, 3).join("|") || "none"}`,
    );
  } catch (err) {
    record("FATAL", "FAIL", err.message);
  } finally {
    await browser.close();
  }

  console.log("\n=== BULK RESULT SAMPLES ===");
  for (const sample of bulkSamples) {
    console.log(
      JSON.stringify({
        operation: "changeRole",
        targetRole: sample.targetRoleLabel,
        total: sample.total,
        succeeded: sample.succeeded,
        skipped: sample.skipped,
        failed: sample.failed,
        summary: sample.summary,
      }),
    );
  }

  console.log("\n=== SMOKE 20.5C.2B SUMMARY ===");
  for (const r of results) console.log(`${r.verdict.padEnd(7)} ${r.id}: ${r.detail}`);

  const fail = results.some((r) => r.verdict === "FAIL");
  console.log(fail ? "\nSMOKE 20.5C.2B: NO-GO" : "\nSMOKE 20.5C.2B: GO");
  process.exit(fail ? 1 : 0);
}

main();
