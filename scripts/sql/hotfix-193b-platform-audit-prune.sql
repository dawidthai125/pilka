-- Sprint 19.3B — prune platformAudit to last 100 entries on RPC write (NOT a new table).
-- Apply manually on Supabase:
--   psql $DATABASE_URL -f scripts/sql/hotfix-193b-platform-audit-prune.sql
-- Includes 19.2B restore rules (onboarding from archived).

BEGIN;

CREATE OR REPLACE FUNCTION public.platform_prune_platform_audit(p_audit JSONB)
RETURNS JSONB
LANGUAGE sql
IMMUTABLE
AS $$
    ...
    (
      ...
      FROM (
        ...
        FROM jsonb_array_elements(
          CASE
            WHEN jsonb_typeof(p_audit) = 'array' THEN p_audit
            ELSE '[]'::jsonb
          END
        ) WITH ORDINALITY AS t(elem, ord)
        WHERE ord > GREATEST(0, jsonb_array_length(
          CASE WHEN jsonb_typeof(p_audit) = 'array' THEN p_audit ELSE '[]'::jsonb END
        ) - 100)
      ) sub
    ),
    '[]'::jsonb
...
BEGIN
  ...
  v_merged := public.platform_prune_platform_audit(v_existing || jsonb_build_array(p_entry));
...
BEGIN
  ...
  IF p_audit_entry IS NOT NULL THEN
    ...
    END IF;
    v_merged := public.platform_prune_platform_audit(v_existing || jsonb_build_array(p_audit_entry));
    v_new_settings := jsonb_set(v_new_settings, '{platformAudit}', v_merged);
  END IF;
...
REVOKE ALL ON FUNCTION public.platform_prune_platform_audit(JSONB) FROM PUBLIC;
