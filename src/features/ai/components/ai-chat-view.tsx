...
const QUICK_PROMPTS = [
  ...
  "Podsumuj formę drużyny z ostatnich 5 meczów.",
  "Kto ma wygasające dokumenty?",
];
...
export function AiChatView({ data }: { data: AiConversationDetail }) {
  ...
  return (
    <div className="flex min-h-[calc(100dvh-10rem)] flex-col gap-4 md:min-h-[calc(100vh-10rem)]">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="min-w-0 flex-1">
          <h1 className="truncate text-xl font-semibold sm:text-2xl">{data.conversation.title}</h1>
      ...
      <div className="flex-1 space-y-3 overflow-y-auto rounded-xl border p-3 sm:p-4">
        {data.messages.map((msg) => (
          <div
            key={msg.id}
            className={cn(
              "max-w-full rounded-lg px-3 py-2 text-sm whitespace-pre-wrap sm:max-w-[85%]",
              msg.role === "user"
      ...
      <form action={formAction} className="sticky bottom-0 flex flex-col gap-2 border-t bg-background pt-3 sm:flex-row">
