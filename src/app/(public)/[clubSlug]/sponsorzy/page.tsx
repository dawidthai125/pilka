...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildPublicPageMetadata("Sponsorzy", clubSlug, "/sponsorzy");
}

const TIERS: WebsiteSponsorTier[] = ["main", "supporting", "partner"];

export default async function SponsorsPublicPage({ params }: Props) {
  const { clubSlug } = await params;
  const sponsors = await getPublicSponsors(clubSlug);

  return (
    <PublicPageShell clubSlug={clubSlug} eyebrow="Partnerzy" title="Partnerzy klubu" subtitle="Firmy wspierające rozwój klubu.">
